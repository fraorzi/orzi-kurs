...
const NEXT_ROOT = join(TRACKS_ROOT, "next");
...
describe("Next content contract", () => {
  it("keeps usage guidance, pitfalls and exact-version sources", () => {
    if (!existsSync(NEXT_ROOT)) return;

    for (const topic of directories(NEXT_ROOT).filter(
  ...
  it("keeps every task typed, solvable and covered by a local test", () => {
    ...
    for (const task of taskDirectories(NEXT_ROOT)) {
      expect(findStarter(task), `${task} nie ma startera`).not.toBeNull();
      expect(findSolution(task), `${task} nie ma rozwiązania`).not.toBeNull();

      const testPath = ["run.test.tsx", "run.test.ts"]
        .map((name) => join(task, name))
        .find(existsSync);
      expect(testPath, `${task} nie ma run.test.tsx/run.test.ts`).toBeDefined();
