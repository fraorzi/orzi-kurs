    ...
    await withMySql(
      ...
      async (connection) => {
        ...
        const index = await rows(
          connection,
          "SELECT COLUMN_NAME AS columnName, SEQ_IN_INDEX AS position FROM information_schema.statistics WHERE table_schema=DATABASE() AND table_name='orders' AND index_name='ix_orders_feed' ORDER BY SEQ_IN_INDEX",
        );
        ...
        const plan = await rows(
          connection,
          "EXPLAIN SELECT id FROM orders FORCE INDEX(ix_orders_feed) WHERE tenant_id=1 AND status='paid' ORDER BY created_at DESC,id DESC",
        );
