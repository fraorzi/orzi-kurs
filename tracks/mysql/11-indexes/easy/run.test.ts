...
describe("Indeks pojedynczej kolumny", () => {
    ...
    await withMySql(
      ...
      async (connection) => {
        ...
        const index = await rows(
          connection,
          "SELECT COLUMN_NAME AS columnName, SEQ_IN_INDEX AS position FROM information_schema.statistics WHERE table_schema=DATABASE() AND table_name='orders' AND index_name='ix_orders_status' ORDER BY SEQ_IN_INDEX",
        );
        expect(index.map((part) => part.columnName)).toEqual(["status"]);
