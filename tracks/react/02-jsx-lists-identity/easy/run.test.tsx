...
describe("InboxSummary", () => {
  it("pokazuje nazwę użytkownika i pusty stan skrzynki", () => {
    ...
    const inbox = screen.getByRole("region", { name: "Skrzynka Ala" });

    expect(within(inbox).getByRole("heading", { name: "Witaj, Ala" }))
      .toBeInTheDocument();
    expect(within(inbox).getByText("Brak nowych wiadomości"))
      .toBeInTheDocument();
  });
  ...
  it("odmienia komunikat dla jednej wiadomości", () => {
    ...
    expect(screen.getByText("1 nowa wiadomość")).toBeInTheDocument();
  });

  it("pokazuje liczbę dla wielu wiadomości", () => {
    render(<InboxSummary userName="Jan" unreadCount={7} />);

    expect(screen.getByText("7 nowych wiadomości")).toBeInTheDocument();
  });
});
