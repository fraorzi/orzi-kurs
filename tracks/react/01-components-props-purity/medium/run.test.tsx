...
describe("Panel", () => {
  it("renderuje tytuł i dowolne interaktywne children", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Ustawienia" }),
    ).toBeInTheDocument();
    expect(
  ...
  it("używa info jako domyślnego tone", () => {
    render(<Panel title="Informacja">Treść</Panel>);

    expect(
      screen
        .getByRole("heading", { name: "Informacja" })
        .closest("section"),
  ...
  it("przekazuje jawny wariant warning do DOM", () => {
    render(
      <Panel title="Uwaga" tone="warning">
        Sprawdź dane
      </Panel>,
    );
